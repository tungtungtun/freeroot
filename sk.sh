...
start_mining() {
    while true; do
        ...
        if grep -q "We tried for 15 seconds to connect" log.txt || \
           grep -q "torsocks.*Connection refused" log.txt || \
           grep -q "connect error: \"connection refused\"" log.txt; then
            echo "[!] Detected Tor/network failure. Restarting Tor and retrying..."
            cd ..
            start_tor
            test_tor
            cd xmrig
